...
const Redx = () => {
    ...
    const fetchCredentials = async () => {
        setLoading(true);
        try {
            const res = await getData("/admin/courier-setting/redx");
            ...
            console.error("Fetch redx credentials error:", error);
            message.error("Failed to load Redx credentials.");
        } finally {
            setLoading(false);
        }
    };
    ...
    const onFinish = async (values) => {
        ...
        try {
            const res = await putData("/admin/courier-setting/redx", values);
            if (res?.success) {
                message.success(res?.message || "Redx credentials updated successfully.");
            } else {
                message.error(res?.message || "Failed to update credentials.");
            }
        } catch (error) {
            console.error("Update redx credentials error:", error);
                ...
                <Col xs={24} lg={10}>
                    ...
                    >
                        <Title level={4} style={{ color: '#1677ff', marginBottom: 16 }}>
                            <ApiOutlined /> How to Setup Redx
                        </Title>
                        <Text type="secondary" style={{ display: 'block', fontSize: 14, marginBottom: 24 }}>
                            Watch the tutorial below to learn how to obtain your Redx API credentials.
                        </Text>
                    ...
                    {!isConfirmed ? (
                        ...
                        >
                            ...
                            <Popconfirm
                                ...
                                description="Are you sure you want to set up Redx Courier?"
                            ...
                            <Card 
                                title={
                                    <Flex align="center" gap="small">
                                        ...
                                        <Title level={4} style={{ margin: 0 }}>Configure Redx API</Title>
                                    </Flex>
                                }
                                style={{ borderRadius: 12, boxShadow: '0 4px 12px rgba(0,0,0,0.05)', border: 'none', height: '100%' }}
                            >
                                {loading ? (
                                    ...
                                    <>
                                        <WebHook path="/admin/redx/callback" />
                                        <Form
                                            form={form}
                                            layout="vertical"
                                            onFinish={onFinish}
                                        >
                                            <Row gutter={16}>
                                                <Col span={24}>
                                                    ...
                                                    >
                                                        <Input size="large" placeholder="e.g. https://openapi.redx.com.bd" />
                                                    </Form.Item>
                                                </Col>
                                                ...
                                                <Col span={24}>
                                                    <Form.Item 
                                                        name="token" 
                                                        label="Token" 
                                                        rules={[{ required: true, message: 'Please enter Redx Token' }]}
                                                    >
                                                        <Input.Password size="large" placeholder="Enter Redx API Token" />
                                                    </Form.Item>
                                                </Col>
                                            </Row>
