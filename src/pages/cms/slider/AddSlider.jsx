...
const AddSlider = () => {
    ...
    const onFinish = async (values) => {
        ...
        try {
            ...
            const response = await postData("/admin/slider", formData);

            if (response?.success !== false) {
                message.success(response?.message || "Slider added successfully");
                navigate("/slider");
            } else {
                message.error(response?.message || "Failed to add slider");
            }
        } catch (error) {
            ...
            <Card
                title={
                    <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                        ...
                        <Title level={4} style={{ margin: 0 }}>Add New Slider</Title>
                    </div>
                }
            >
