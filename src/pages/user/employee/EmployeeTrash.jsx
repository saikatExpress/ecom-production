...
const EmployeeTrash = () => {
    ...
    const getEmployeeTrash = async (page = 1, paginate_size = 25, search_key = filters.search_key, status = filters.status) => {
        try {
            setLoading(true);
    ...
    useEffect(() => {
        getEmployeeTrash(pagination.current, pagination.pageSize, filters.search_key, filters.status);
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, []);

    const handleSearch = (value) => {
        setFilters(prev => ({ ...prev, search_key: value }));
        getEmployeeTrash(1, pagination.pageSize, value, filters.status);
    };

    const handleStatusChange = (value) => {
        setFilters(prev => ({ ...prev, status: value }));
        getEmployeeTrash(1, pagination.pageSize, filters.search_key, value);
    };

    const handleTableChange = (newPagination) => {
        getEmployeeTrash(newPagination.current, newPagination.pageSize, filters.search_key, filters.status);
    };
