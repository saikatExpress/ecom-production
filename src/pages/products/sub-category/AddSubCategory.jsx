...
const AddSubCategory = () => {
    ...
    const onFinish = async (values) => {
        ...
        try {
            ...
            const res = await postData("/admin/subcategory", formData);
            
            if (res?.success) {
                message.success(res?.message || "Subcategory created successfully!");
                navigate(-1);
            } else {
                message.error(res?.message || "Failed to create subcategory");
            }
        } catch (error) {
            console.error(error);
