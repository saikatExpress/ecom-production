...
const EditCoupon = () => {
    ...
    const fetchCouponData = async () => {
        setFetching(true);
        try {
            const res = await getData(`/admin/coupon/${id}`);
            ...
            if (data) {
                ...
                if (data.apply_scope === "selected_categories" && data.categories) {
                    form.setFieldsValue({
    ...
    const onFinish = async (values) => {
        ...
        try {
            ...
            const response = await putData(`/admin/coupon/${id}`, payload);

            if (response?.success !== false) {
                message.success(response?.message || "Coupon updated successfully");
                navigate(-1);
            } else {
                message.error(response?.message || "Failed to update coupon");
            }
        } catch (error) {
                                                        ...
                                                        <List
                                                            ...
                                                            renderItem={(item) => (
                                                                <List.Item
                                                                    key={item.id}
                                                                    actions={[
                                                                        <Button type="primary" size="small" icon={<PlusOutlined />} onClick={() => handleAddProduct(item)}
